...
class ConnectionManager extends AbstractConnectionManager {
  ...
  connect(config) {
    ...
    let responded = false;
    function makeEndListener(reject) {
        return () => {
    ...
    function makeErrorListener(connection) {
      return err => {
    ...
    return new Promise((resolve, reject) => {
      ...
      connection.on('end', makeEndListener(reject));

      // Don't let a Postgres restart (or error) to take down the whole app
      connection.on('error', makeErrorListener(connection));
    }).tap(connection => {
